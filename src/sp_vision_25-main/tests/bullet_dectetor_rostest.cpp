...
// const double AIR_RESISTANCE_FACTOR = 0.0095; // 空气阻力因子
...
// const Eigen::Vector3d LAUNCH_ORIGIN_OFFSET(0.05, 0.01, -0.1); // 空间偏差 (米)xyz右下前
// const Eigen::Vector3d LAUNCH_DIRECTION_OFFSET(0.0, 0.03, 0.0); // 朝向偏差 (弧度)ypr
...
public:
    ...
    // 计算理想弹道上的点（相机坐标系）
    // 直接在相机坐标系中计算，不考虑相机朝向，排除roll和pitch的影响
    Eigen::Vector3d calculateIdealTrajectoryPoint(double time, double initial_speed) {
        ...
        // 修正速度分量计算：
        // 在相机坐标系中，z轴向前，x轴向右，y轴向下
        // 因此：
        // - 向前速度（z轴）由cos(pitch)和cos(yaw)决定
        // - 向右速度（x轴）由sin(yaw)决定  
        // - 向下速度（y轴）由sin(pitch)决定（注意：在相机系中向下为正）
    ...
    void drawIdealTrajectory(cv::Mat& img, double initial_speed, double max_time = -1.0, int num_points = 50) {
        ...
        if (debug_count < 5) {
            ...
            tools::logger()->info("Launch origin offset: ({:.3f}, {:.3f}, {:.3f})", 
                                  LAUNCH_ORIGIN_OFFSET.x(), LAUNCH_ORIGIN_OFFSET.y(), LAUNCH_ORIGIN_OFFSET.z());
            tools::logger()->info("Launch direction offset: ({:.3f}, {:.3f}, {:.3f}) rad", 
                                  LAUNCH_DIRECTION_OFFSET.x(), LAUNCH_DIRECTION_OFFSET.y(), LAUNCH_DIRECTION_OFFSET.z());
        }
        ...
        for (int i = 0; i <= num_points; ++i) {
            ...
            // 输出3个关键点的3D坐标
            if (debug_count < 5 && (i == 0 || i == num_points/2 || i == num_points)) {
                tools::logger()->info("Time {:.2f}s: cam pos ({:.3f}, {:.3f}, {:.3f})", 
                                      time, position_cam.x(), position_cam.y(), position_cam.z());
            }
            ...
            // 输出3个关键点的2D坐标
            if (debug_count < 5 && (i == 0 || i == num_points/2 || i == num_points)) {
                tools::logger()->info("Projected 2D position: ({:.2f}, {:.2f})", image_pos.x, image_pos.y);
            }
            ...
            if (image_pos.x >= 0 && image_pos.x < img.cols && image_pos.y >= 0 && image_pos.y < img.rows) {
                ...
                if (debug_count < 5 && (i == 0 || i == num_points/2 || i == num_points)) {
                    tools::logger()->info("Point added to trajectory");
                }
            } else {
                if (debug_count < 5 && (i == 0 || i == num_points/2 || i == num_points)) {
                    tools::logger()->info("Point outside image bounds, skipped");
                }
            }
        }
        ...
        // 输出调试信息
        if (debug_count < 5) {
            tools::logger()->info("Total points calculated: {}", num_points + 1);
            tools::logger()->info("Points within image: {}", trajectory_points.size());
            if (!trajectory_points.empty()) {
                tools::logger()->info("First point: ({:.2f},{:.2f})", trajectory_points.front().x, trajectory_points.front().y);
                tools::logger()->info("Last point: ({:.2f},{:.2f})", trajectory_points.back().x, trajectory_points.back().y);
            }
            debug_count++;
        }
        ...
        if (trajectory_points.size() > 1) {
            if (debug_count < 5) {
                tools::logger()->info("Drawing trajectory with {} points", trajectory_points.size());
            }
            ...
            if (debug_count < 5) {
                tools::logger()->info("Not enough points to draw trajectory");
            }
        }
    }
...
public:
  ROS2Publisher(ProjectileTrajectoryPredictor* trajectory_predictor = nullptr)
    : Node("bullet_detector_test_publisher")
...
int main(int argc, char * argv[])
{
  ...
  for (int frame_count = start_index; !exiter.exit(); frame_count++) {
    ...
    Eigen::Quaternion q2{w, x, y, z};
    auto yaw = tools::eulers(q2, 2, 1, 0)[0];
    ...
    if (!targets.empty()) {
      ...
      ros2_publisher->publish_marker(true, position, velocity, angular_velocity, armor_positions, armor_yaws, q_gimbal);
    } else {
      // 发布空标记
  ...
  converter.update(img.clone(), q_gimbal, t);
    ...
    // 应用延迟补偿
    double compensated_time = t + SYSTEM_LATENCY;
    ...
    if (!detected_bullets.empty()) {
      ...
      for (const auto& bullet : detected_bullets) {
        // 通过弹丸大小估算距离
        // 原理：弹丸在图像中的大小与距离成反比
        double focal_length = converter.get_f_cv_mat_ref().at<double>(0, 0); // 获取焦距
        ...
        if (bullet.center.x >= 0 && bullet.center.y >= 0 && 
          ...
          double brightness = pixel[2]; // V通道亮度
          double saturation = pixel[1]; // S通道饱和度
        ...
        // 为弹丸创建虚拟ID（寻找匹配的历史弹丸或创建新ID）
        int matched_bullet_id = -1;
        double min_distance = BULLET_MATCH_DISTANCE_THRESHOLD; // 阈值像素距离
        ...
        if (matched_bullet_id != -1 && min_distance < BULLET_MATCH_DISTANCE_THRESHOLD) {
          ...
          // 计算时间差和距离差
          double time_diff = t - history.last_time;
          if (time_diff > 0 && time_diff < BULLET_MAX_TIME_DIFF) { // 确保时间差合理
            ...
            // 检查速度是否在有效范围内（放宽松）
            bool speed_valid = (speed >= MIN_BULLET_SPEED - 2.0 && speed <= MAX_BULLET_SPEED + 10.0); // 放宽松
            
            if (speed_valid || history.consecutive_detections < 3) { // 对新弹丸更宽松
          ...
          BulletHistory new_history(bullet.center, t, estimated_distance);
          
          // 计算初始速度
          new_history.estimated_speed = BULLET_V0; // 使用配置的初始速度
    ...
    std::vector<int> to_remove;
    for (const auto& entry : bullet_histories) {
      if (t - entry.second.last_time > 1.0) { // 1秒没有检测到就删除
